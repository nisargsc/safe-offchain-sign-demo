...
import * as safeUtils from "./utils/safeUtils";
import { apiRespType } from "./Types";

dotenv.config();

async function main() {
  const apiUrl = "http://backend.dev.buildbear.io";
  const nodeId = "absolute-taun-we-0bf9874a"
  const rpcUrl = "https://rpc.dev.buildbear.io/absolute-taun-we-0bf9874a"
  ...
  const ethAdapterOwner2 = safeUtils.getEthAdapter(owner2);
  const ethAdapterOwner3 = safeUtils.getEthAdapter(owner3);
  ...
  const contractNetworks = safeUtils.getContractNetworks(chainId);
  console.log({ contractNetworks });

  // Comment this part if already deployed and Un-comment the section below
  ...
  // Un-comment this part if already deployed and comment the section above
  ...
  const execTxn = await safeUtils.getExecTxn(
    ethAdapterOwner1,
    safeAddress,
    getTxnRespOwner1
  );
